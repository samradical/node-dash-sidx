...
var SidxInterface = (() => {
  ...
  function start(options) {
    ...
    var _cmd = `${youtubeDlPath} ${VIDEO_BASE}${id} -F`
    console.log(_cmd);

    return new Q((yes, no) => {

      const child = spawn(`${youtubeDlPath}`, [`${VIDEO_BASE}${id}`, `-F`])
      ...
      var _c = `${youtubeDlPath}  ${VIDEO_BASE}${id} --skip-download -v --write-pages`
      ...
      let mpdSpawn = spawn(`${youtubeDlPath}`, [`${VIDEO_BASE}${id}`, `--skip-download`, '-v', '--write-pages'])
