...
var SidxInterface = (() => {
  ...
  function start(options) {
    ...
    return new Q((yes, no) => {
      ...
      if (loginCommand.length)
        args = args.concat(loginCommand.split(" "))
      ...
      if (!desiredTags.length) {
        desiredTags = options.audioiTags
          ? [...DASH_AUDIO_TAGS]
          : [...DASH_VIDEO_TAGS]
      }
    ...
    */
      ...
      let mpdSpawn = spawn(
        ...
        [
          ...
          "--write-pages",
          "--no-warnings",
        ].concat(loginCommand.split(" "))
      )

      var parser = new xml2js.Parser()
      const parseString = Q.promisify(parser.parseString)
      var filesArray = readDir.readSync(
        process.cwd(),
        ["**.dump"],
      ...
      yes(
        Q.map(filesArray, p => {
          return readFile(p, "utf-8").then(data => {
                  ...
                  if (result.MPD) {
                    ...
                    var _reps = flatten(
                      adpatation.map(av => {
                        return av.Representation.filter(rep => {
                          ...
                          var repVars = rep.$
                          if (
                            desiredTags.indexOf(repVars.id) > -1 &&
                            foundItags.indexOf(repVars.id) < 0
                          ) {
                            foundItags.push(repVars.id)
                            var segmentList =
                              rep.SegmentList || rep.SegmentBase
                            if (segmentList) {
                              _r = true
                            }
                          }
                    ...
                    if (_reps.length) {
                      return Q.map(_reps, rep => {
                        ...
                        var indexRange
                        if (segmentList[0].$) {
                          let _ii = segmentList[0].$.indexRange.split(
                            "-"
                          )[1]
                          if (isNaN(_ii)) {
                            return null
                          ...
                          var _i = segmentList[0].SegmentURL[0].$.media
                            .replace("range/", "")
                            .split("-")[0]
          ...
          if (r.length) {
            ...
            return _bruteReading(filesArray, desiredTags, id).then(
              datas => {
                _deleteMpds(filesArray, id)
                if (datas.length) {
                  return Q.map(datas, data => getSidx(data))
                } else {
                  return _getInfo(desiredTags, id).catch(err => {
                    const index = DASH_VIDEO_TAGS.indexOf(
                      desiredTags[desiredTags.length - 1]
                    )
                    const targetItag =
                      DASH_VIDEO_TAGS[
                        DASH_VIDEO_TAGS.indexOf(
                          desiredTags[desiredTags.length - 1]
                        ) + 1
                      ]
                    if (targetItag) {
        ...
        .then(data => {
          ...
          return Q.map(desiredTags, itag => {
            ...
            const chunks = split.filter(chunk => {
              return (
                chunk.indexOf(`&itag=${itag}`) > -1 &&
                chunk.indexOf(`/mp4`) > -1
              )
            })

            return Q.map(chunks, chunk => {
              return getURL(id, itag).then(url => {
